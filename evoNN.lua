...
function Population:evolve(gen)
  ...
  while #nextGen < cfg.popSize do
    ...
    local child = (cfg.crossoverType == "onepoint") and onePointCrossover(a, b) or NeuralNetwork.crossover(a, b)
...
function M.train(cfg, sim)
  ...
  for gen = 1, cfg.generations do
    pop:evaluate(sim)
    local sum, mx = 0, pop.fitness[1].score
    for _, r in ipairs(pop.fitness) do sum = sum + r.score end
    local mean = sum / #pop.fitness
